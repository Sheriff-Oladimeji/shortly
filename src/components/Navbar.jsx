...
const Navbar = () => {
  ...
  return (
    <header className="w-full font-Poppins ">
      <nav className="hidden md:flex justify-between w-[80%] mx-auto py-8 text-[16px]">
      ...
      <nav className="md:hidden relative h-[200px] text-[18px]">
        ...
        <ul
          ...
          } w-[90%] mx-auto my-1  bg-DarkViolet text-[#fff]  rounded-xl space-y-6 flex flex-col items-center py-8 z-50 transform duration-300 relative h-[400px] justify-between font-bold`}
        >
          {navLinks.map((link) => (
            <li key={link.id}>
              <a href={`#${link.content}`}>{link.content}</a>
          ...
          {CTA.map((link) => (
            <li
              ...
              className={`${
                link.id === 2
                  ? " bg-Cyan rounded-full py-2 w-[90%] text-[#FFf] text-center "
                  : ""
              }`}
            >
              <a href={`#${link.content}`}>{link.content}</a>
